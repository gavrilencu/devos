#include "aes.h"
#include "string.h"

static const uint8_t sbox[256] = {
    ,
};

static uint8_t xtime(uint8_t x) { return (uint8_t)((x << 1) ^ ((x >> 7) * 0x1b)); }

static uint8_t mul(uint8_t a, uint8_t b)
{
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        uint8_t hi = a & 0x80;
        a <<= 1;
        if (hi) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

void aes_init(aes_ctx *c, const uint8_t *key, int keybits)
{
    int nk = keybits / 32;             /* 4 sau 8 */
    c->nr = nk + 6;                    /* 10 sau 14 */
    int total = 4 * (c->nr + 1);       /* cuvinte de 4 octeti */
    uint8_t *rk = c->rk;
    memcpy(rk, key, nk * 4);
    uint8_t rcon = 1;
    for (int i = nk; i < total; i++) {
        uint8_t t[4];
        memcpy(t, rk + (i - 1) * 4, 4);
        if (i % nk == 0) {
            uint8_t tmp = t[0];
            t[0] = (uint8_t)(sbox[t[1]] ^ rcon);
            t[1] = sbox[t[2]];
            t[2] = sbox[t[3]];
            t[3] = sbox[tmp];
            rcon = xtime(rcon);
        } else if (nk > 6 && i % nk == 4) {
            for (int j = 0; j < 4; j++) t[j] = sbox[t[j]];
        }
        for (int j = 0; j < 4; j++)
            rk[i * 4 + j] = (uint8_t)(rk[(i - nk) * 4 + j] ^ t[j]);
    }
}

void aes_encrypt(const aes_ctx *c, const uint8_t in[16], uint8_t out[16])
{
    uint8_t s[16];
    memcpy(s, in, 16);
    const uint8_t *rk = c->rk;
    for (int i = 0; i < 16; i++) s[i] ^= rk[i];
    for (int round = 1; round <= c->nr; round++) {
        for (int i = 0; i < 16; i++) s[i] = sbox[s[i]];
        /* ShiftRows */
        uint8_t t[16];
        t[0] = s[0];  t[4] = s[4];  t[8]  = s[8];   t[12] = s[12];
        t[1] = s[5];  t[5] = s[9];  t[9]  = s[13];  t[13] = s[1];
        t[2] = s[10]; t[6] = s[14]; t[10] = s[2];   t[14] = s[6];
        t[3] = s[15]; t[7] = s[3];  t[11] = s[7];   t[15] = s[11];
        memcpy(s, t, 16);
        if (round < c->nr) {
            for (int col = 0; col < 4; col++) {
                uint8_t *p = s + col * 4;
                uint8_t a0 = p[0], a1 = p[1], a2 = p[2], a3 = p[3];
                p[0] = (uint8_t)(mul(a0, 2) ^ mul(a1, 3) ^ a2 ^ a3);
                p[1] = (uint8_t)(a0 ^ mul(a1, 2) ^ mul(a2, 3) ^ a3);
                p[2] = (uint8_t)(a0 ^ a1 ^ mul(a2, 2) ^ mul(a3, 3));
                p[3] = (uint8_t)(mul(a0, 3) ^ a1 ^ a2 ^ mul(a3, 2));
            }
        }
        for (int i = 0; i < 16; i++) s[i] ^= rk[round * 16 + i];
    }
    memcpy(out, s, 16);
}

/* ---- GCM ---- */

/* multiplicare în GF(2^128), reprezentarea GCM (bit cel mai semnificativ
 * primul, polinom de reducere 0xe1 pe octetul de sus) */
static void ghash_mul(uint8_t *x, const uint8_t *h)
{
    uint8_t z[16];
    memset(z, 0, 16);
    uint8_t v[16];
    memcpy(v, h, 16);
    for (int i = 0; i < 128; i++) {
        int bit = (x[i / 8] >> (7 - (i % 8))) & 1;
        if (bit)
            for (int j = 0; j < 16; j++) z[j] ^= v[j];
        int lsb = v[15] & 1;
        for (int j = 15; j > 0; j--)
            v[j] = (uint8_t)((v[j] >> 1) | (v[j - 1] << 7));
        v[0] >>= 1;
        if (lsb) v[0] ^= 0xe1;
    }
    memcpy(x, z, 16);
}

static void ghash(const uint8_t *h, const uint8_t *aad, size_t aadlen,
                  const uint8_t *cipher, size_t clen, uint8_t out[16])
{
    uint8_t y[16];
    memset(y, 0, 16);
    size_t i;
    for (i = 0; i + 16 <= aadlen; i += 16) {
        for (int j = 0; j < 16; j++) y[j] ^= aad[i + j];
        ghash_mul(y, h);
    }
    if (i < aadlen) {
        for (size_t j = 0; j < aadlen - i; j++) y[j] ^= aad[i + j];
        ghash_mul(y, h);
    }
    for (i = 0; i + 16 <= clen; i += 16) {
        for (int j = 0; j < 16; j++) y[j] ^= cipher[i + j];
        ghash_mul(y, h);
    }
    if (i < clen) {
        for (size_t j = 0; j < clen - i; j++) y[j] ^= cipher[i + j];
        ghash_mul(y, h);
    }
    uint8_t lenblk[16];
    uint64_t abits = (uint64_t)aadlen * 8, cbits = (uint64_t)clen * 8;
    for (int j = 0; j < 8; j++) lenblk[j] = (uint8_t)(abits >> (56 - j * 8));
    for (int j = 0; j < 8; j++) lenblk[8 + j] = (uint8_t)(cbits >> (56 - j * 8));
    for (int j = 0; j < 16; j++) y[j] ^= lenblk[j];
    ghash_mul(y, h);
    memcpy(out, y, 16);
}

static void inc32(uint8_t *ctr)
{
    for (int i = 15; i >= 12; i--) {
        if (++ctr[i]) break;
    }
}

static void gcm_crypt(const aes_ctx *c, const uint8_t j0[16],
                      const uint8_t *in, size_t len, uint8_t *out)
{
    uint8_t ctr[16], ks[16];
    memcpy(ctr, j0, 16);
    for (size_t i = 0; i < len; i += 16) {
        inc32(ctr);
        aes_encrypt(c, ctr, ks);
        size_t n = len - i < 16 ? len - i : 16;
        for (size_t j = 0; j < n; j++) out[i + j] = in[i + j] ^ ks[j];
    }
}

static void gcm_core(const uint8_t *key, int keybits, const uint8_t iv[12],
                     const uint8_t *aad, size_t aadlen,
                     const uint8_t *in, size_t len, uint8_t *out,
                     uint8_t tag[16], int encrypt)
{
    aes_ctx c;
    aes_init(&c, key, keybits);
    uint8_t h[16], zero[16];
    memset(zero, 0, 16);
    aes_encrypt(&c, zero, h);
    uint8_t j0[16];
    memcpy(j0, iv, 12);
    j0[12] = 0; j0[13] = 0; j0[14] = 0; j0[15] = 1;

    /* pentru criptare: cifrăm, apoi GHASH pe cifru; pentru decriptare:
     * GHASH pe cifru (in), apoi decifrăm */
    const uint8_t *cipher = encrypt ? out : in;
    gcm_crypt(&c, j0, in, len, out);
    uint8_t s[16];
    ghash(h, aad, aadlen, cipher, len, s);
    uint8_t ej0[16];
    aes_encrypt(&c, j0, ej0);
    for (int i = 0; i < 16; i++) tag[i] = s[i] ^ ej0[i];
}

void aes_gcm_encrypt(const uint8_t *key, int keybits, const uint8_t iv[12],
                     const uint8_t *aad, size_t aadlen,
                     const uint8_t *plain, size_t plen,
                     uint8_t *cipher, uint8_t tag[16])
{
    gcm_core(key, keybits, iv, aad, aadlen, plain, plen, cipher, tag, 1);
}

int aes_gcm_decrypt(const uint8_t *key, int keybits, const uint8_t iv[12],
                    const uint8_t *aad, size_t aadlen,
                    const uint8_t *cipher, size_t clen,
                    const uint8_t tag[16], uint8_t *plain)
{
    /* recalculăm tag-ul din cifru înainte de a-l suprascrie */
    aes_ctx c;
    aes_init(&c, key, keybits);
    uint8_t h[16], zero[16];
    memset(zero, 0, 16);
    aes_encrypt(&c, zero, h);
    uint8_t j0[16];
    memcpy(j0, iv, 12);
    j0[12] = 0; j0[13] = 0; j0[14] = 0; j0[15] = 1;
    uint8_t s[16];
    ghash(h, aad, aadlen, cipher, clen, s);
    uint8_t ej0[16], want[16];
    aes_encrypt(&c, j0, ej0);
    for (int i = 0; i < 16; i++) want[i] = s[i] ^ ej0[i];
    int diff = 0;
    for (int i = 0; i < 16; i++) diff |= want[i] ^ tag[i];
    if (diff) return -1;
    gcm_crypt(&c, j0, cipher, clen, plain);
    return 0;
}
