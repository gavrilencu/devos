...
void kmain(void)
{
    ...
    kprintf("%s v%s (%s)  build %s\n\n",
            KERNEL_NAME, KERNEL_VERSION, KERNEL_ARCH, KERNEL_BUILD_ID);
    ...
    acpi_init();              /* Milestone 56: descopera nucleele CPU + APIC */
    if (lapic_init())
        KINFO("apic", "Local APIC activat; nucleul de boot are APIC ID %u",
              (unsigned)lapic_id());
    ...
    slab_selftest();          /* Milestone 55: allocator slab (object cache) */
    KINFO("mm", "PMM %llu/%llu MiB libere, kheap %llu/%llu KiB libere",
          (unsigned long long)(pmm_free_bytes() / (1024 * 1024)),
          (unsigned long long)(pmm_managed_bytes() / (1024 * 1024)),
    ...
    KINFO("kernel", "%s v%s (%s) build %s - subsisteme: mm sched fs net ipc drivers",
          KERNEL_NAME, KERNEL_VERSION, KERNEL_ARCH, KERNEL_BUILD_ID);
