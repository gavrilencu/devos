...
/* Decodeaza codul de eroare al unui #PF (bitii P/W/U/I) intr-un mesaj clar. */
static void pf_decode(uint64_t err, uint64_t cr2)
{
    kprintf("[kernel]   cauza: %s, %s, %s%s\n",
            (err & 1)  ? "protectie (pagina prezenta)" : "pagina lipsa",
            (err & 2)  ? "la scriere" : "la citire",
            (err & 4)  ? "acces din ring 3" : "acces din kernel",
            (err & 16) ? ", aducere instructiune (NX?)" : "");
    if (task_addr_in_user_stack(cr2))
        kprintf("[kernel]   -> STACK OVERFLOW (a lovit zona de garda a stivei)\n");
}
    ...
    if (f->vector < 32) {
        ...
        if ((f->cs & 3) == 3) {
            if (f->vector == 14) {
                uint64_t cr2;
                __asm__ volatile("mov %%cr2, %0" : "=r"(cr2));
                /* Page fault de pagina LIPSA (bit 0 = 0): incearca sa creasca
                 * stiva la cerere. Daca reuseste, reia instructiunea. */
                if (!(f->error & 1) && task_user_fault(cr2))
                    return (uint64_t)f;
                con_color(VGA_LIGHT_RED, VGA_BLACK);
                kprintf("\n[kernel] Task user omorat: #PF la %p (RIP=%p)\n",
                        (void *)cr2, (void *)f->rip);
                pf_decode(f->error, cr2);
