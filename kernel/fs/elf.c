...
/* Biti de permisiune din p_flags (RWX). */
#define PF_X 1
#define PF_W 2
#define PF_R 4
             ...
             uint64_t *entry_out)
{
    ...
    for (int i = 0; i < eh->e_phnum; i++) {
        ...
        /* Permisiuni W^X din p_flags: cod = R-X (executabil, ne-scriabil),
         * date = RW + NX. Segmentele sunt aliniate la pagina (user.ld
         * -z max-page-size=0x1000), deci nu se amesteca pe aceeasi pagina. */
        uint64_t vflags = VMM_U;
        if (ph[i].p_flags & PF_W)
            vflags |= VMM_W;
        if (!(ph[i].p_flags & PF_X))
            vflags |= VMM_NX;                /* ce nu e cod nu se executa */
        ...
        for (uint64_t page = start; page < end; page += PMM_FRAME_SIZE) {
            ...
            if (vmm_map_in(space, page, frame, vflags) < 0)
                return -1;
        }
