...
/* Stiva user poate CRESTE la cerere (demand paging) pana la aceasta limita.
 * Sub ea e o zona de garda (nemapata): un stack overflow o loveste si procesul
 * e oprit curat. 512 pagini = 2 MiB. */
#define USER_STACK_MAX_PAGES 512
#define USER_STACK_LIMIT (USER_STACK_TOP - (uint64_t)USER_STACK_MAX_PAGES * PMM_FRAME_SIZE)
                     ...
                     const char *args, int term)
{
    ...
    for (uint32_t i = 0; i < USER_STACK_PAGES; i++) {
        ...
        vmm_map_in(space, USER_STACK_TOP - (uint64_t)(i + 1) * PMM_FRAME_SIZE,
                   frame, VMM_W | VMM_U | VMM_NX);   /* stiva nu se executa */
    }
 ...
 * Se cheama din tratarea #PF DOAR pentru fault de pagina lipsa (not-present).
 * Intoarce 1 daca a mapat o pagina noua (instructiunea se reia), 0 altfel
 * (adresa e in afara stivei / sub zona de garda -> proces oprit). */
int task_user_fault(uint64_t addr)
{
    struct task *t = &tasks[current];
    if (!t->user || !t->space)
        return 0;
    /* doar in intervalul stivei care poate creste, deasupra zonei de garda */
    if (addr < USER_STACK_LIMIT || addr >= USER_STACK_TOP)
        return 0;
    uint64_t page = addr & ~0xFFFull;
    if (vmm_translate_in(t->space, page) != VMM_NOT_MAPPED)
        return 0;                            /* deja mapata -> fault real */

    uint64_t frame = pmm_alloc();
    if (frame == 0)
        return 0;
    memset((void *)frame, 0, PMM_FRAME_SIZE);
    if (vmm_map_in(t->space, page, frame, VMM_W | VMM_U | VMM_NX) < 0) {
        pmm_free(frame);
        return 0;
    }
    t->mem_kb += PMM_FRAME_SIZE / 1024;      /* stiva a crescut */
    return 1;
}

/* Adresa in interiorul stivei user (pentru mesaje de stack overflow). */
int task_addr_in_user_stack(uint64_t addr)
{
    return tasks[current].user &&
           addr >= USER_STACK_LIMIT - PMM_FRAME_SIZE && addr < USER_STACK_TOP;
}
