...
static uint64_t rover;                   /* Milestone 55: hint next-fit pentru
                                          * pmm_alloc (evita scanarea de la 0) */
...
{
    /* Cautam de la `rover` (next-fit): in regim normal cadrul urmator e liber,
     * deci alocarea e O(1) amortizat in loc de O(n) de la zero. */
...
void pmm_free(uint64_t frame_addr)
{
    ...
    set_free(f);
    free_frames++;
    if (f < rover)               /* refolosim curand cadrul eliberat */
        rover = f;
}
